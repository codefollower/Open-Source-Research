...
{
	private static my.Debug DEBUG=new my.Debug(my.Debug.PGStream);//我加上的
    ...
    public void changeSocket(Socket socket) throws IOException {
		try {//我加上的
		DEBUG.P(this,"changeSocket(1)");
		DEBUG.P("socket="+socket);
    ...
    public void setEncoding(Encoding encoding) throws IOException {
		try {//我加上的
		DEBUG.P(this,"setEncoding(1)");
		DEBUG.P("encoding="+encoding);
		DEBUG.P("encodingWriter="+encodingWriter);
    ...
    {
		try {//我加上的
		DEBUG.P(this,"ReceiveTupleV3()");

        //TODO: use l_msgSize
        int l_msgSize = ReceiveInteger4(); //一条记录总字节数+4

		DEBUG.P("l_msgSize="+l_msgSize);

        int i;
        int l_nf = ReceiveInteger2();

		DEBUG.P("l_nf="+l_nf); //字段数
        ...
        for (i = 0 ; i < l_nf ; ++i)
        {
            int l_size = ReceiveInteger4(); //字段字节数

			DEBUG.P("l_size="+l_size);

            if (l_size != -1) {
                try {
                    answer[i] = new byte[l_size];
                    Receive(answer[i], 0, l_size);
					DEBUG.P("answer[i]="+new String(answer[i]));
