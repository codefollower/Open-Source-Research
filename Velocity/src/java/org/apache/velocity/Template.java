...
//第一步:
//在Velocity.getTemplate时会调用process()，
//然后在process()过程中生成一棵抽象语法树AST，AST的最顶层的节点是ASTprocess，
//接着调用((SimpleNode)data).init( ica, rsvc);，从顶层的节点开始初始化整棵树.

//第二步:
//调用Template.merge方法，然后( (SimpleNode) data ).render( ica, writer);
//从顶层的节点开始render，结果写入writer中
public class Template extends Resource
{
        ...
        throws ResourceNotFoundException, ParseErrorException, IOException
    {
        ...
        if (is != null)
        {
            ...
            try
            {
				...
				//指:org.apache.velocity.runtime.parser.node.ASTprocess
				if(data!=null) DEBUG.P("data.getClass()="+data.getClass());
    ...
    {
		try {//我加上的
		DEBUG.P(this,"merge(2)");

        merge(context, writer, null);

		}finally{//我加上的
		DEBUG.P(0,this,"merge(2)");
		}
    }
