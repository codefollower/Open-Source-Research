 ...
 * @version $Id: RuntimeInstance.java 896109 2010-01-05 15:59:48Z nbubna $
 */
public class RuntimeInstance implements RuntimeConstants, RuntimeServices
{
     ...
     */
	//用户可以通过程序的方式指定一个properties文件，
	//文件内容会存放到这个ExtendedProperties类中(是Properties的子类)
	...
	//最先是读org\apache\velocity\runtime\defaults\velocity.properties
	//但是会被overridingProperties中用户指定的新值覆盖
    ...
    public RuntimeInstance()
    {
		...
		DEBUG.P(0,this,"RuntimeInstance()");
    }
        ...
        throws Exception
    {
		try {//我加上的
		DEBUG.P(this,"init()");
		DEBUG.P("initialized="+initialized);
		DEBUG.P("initializing="+initializing);
        ...
        if (!initialized && !initializing)
        {
			...
			DEBUG.P("initializing1="+initializing);

            initializeEventHandlers();

			DEBUG.P("initializing2="+initializing);

            initializeParserPool();

			DEBUG.P("initializing3="+initializing);
    ...
    {
		//我加上的，不然会在ParserPoolImpl===>initialize(1)时一直调用init(20次)
    ...
    {
		try {//我加上的
		DEBUG.P(this,"initializeIntrospection()");
        ...
        for (int i=0; i <uberspectors.length;i++)
        {
            ...
            {
				DEBUG.P("(u instanceof ChainableUberspector)="+(u instanceof ChainableUberspector));
		...
		DEBUG.P(0,this,"initializeIntrospection()");
		}
    }
    ...
    {
		try {//我加上的
		DEBUG.P(this,"setDefaultProperties()");
		...
		DEBUG.P(0,this,"setDefaultProperties()");
		}
    }
    ...
    {
		try {//我加上的
		DEBUG.P(this,"initializeProperties()");
		DEBUG.P("configuration.isInitialized()="+configuration.isInitialized());
        ...
        {
			//先读\org\apache\velocity\runtime\defaults\velocity.properties
            setDefaultProperties();
        }

        DEBUG.P("overridingProperties="+overridingProperties);
		...
		DEBUG.P(0,this,"initializeProperties()");
		}
    }
    ...
    {
        try {//我加上的
		DEBUG.P(this,"init(Properties p)");

        setProperties(ExtendedProperties.convertProperties(p));
        init();

		}finally{//我加上的
		DEBUG.P(0,this,"init(Properties p)");
		}
    }
        ...
        throws Exception
    {
        try {//我加上的
		DEBUG.P(this,"init(1)");
		DEBUG.P("configurationFile="+configurationFile);

        setProperties(new ExtendedProperties(configurationFile));
        init();

		}finally{//我加上的
		DEBUG.P(0,this,"init(1)");
		}
    }
    ...
    {
        try {//我加上的
		DEBUG.P(this,"initializeResourceManager()");
		...
		DEBUG.P("rm="+rm); //org.apache.velocity.runtime.resource.ResourceManagerImpl
		...
		DEBUG.P(0,this,"initializeResourceManager()");
		}
    }
        ...
        throws Exception
    {
		...
		//5种事件处理器，见org.apache.velocity.app.event包
	...
	//加载8个预定义指令，用userdirective属性指定的用户指令也会加载
    ...
    {
		try {//我加上的
		DEBUG.P(this,"initializeParserPool()");
		...
		DEBUG.P(0,this,"initializeParserPool()");
		}
    ...
    {
		try {//我加上的
		DEBUG.P(this,"parse(3)");
		DEBUG.P("templateName="+templateName);
		DEBUG.P("dumpNamespace="+dumpNamespace);
    ...
    {
		try {//我加上的
		DEBUG.P(this,"invokeVelocimacro(5)");
		...
		DEBUG.P("template="+template.toString()); //#tablerows( $color $list )

        return evaluate(context, writer, logTag, template.toString());

		}finally{//我加上的
		DEBUG.P(0,this,"invokeVelocimacro(5)");
		}
    }
