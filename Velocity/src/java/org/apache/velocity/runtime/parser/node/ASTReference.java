...
{
	private static my.Debug DEBUG=new my.Debug(my.Debug.ASTReference);//我加上的

    /* Reference types */
    private static final int NORMAL_REFERENCE = 1; //如$var
    private static final int FORMAL_REFERENCE = 2; //如${var}
    private static final int QUIET_REFERENCE = 3; //如$!var
    private static final int RUNT = 4; //内部parser问题，把标识符当成引用

	String RT() {
		if(referenceType==NORMAL_REFERENCE) return "NORMAL_REFERENCE";
		else if(referenceType==FORMAL_REFERENCE) return "FORMAL_REFERENCE";
		else if(referenceType==QUIET_REFERENCE) return "QUIET_REFERENCE";
		else if(referenceType==RUNT) return "RUNT";
		else return "referenceType="+referenceType;
	}
    ...
    throws TemplateInitException
    {
		...
		//我加上的
		//new org.apache.velocity.runtime.visitor.NodeViewMode().visit(this, data);
		...
		DEBUG.P("rootString="+rootString);

        numChildren = jjtGetNumChildren();

		DEBUG.P("numChildren="+numChildren);

        /*
         * and if appropriate...
         */

		DEBUG.P("identifier1="+identifier);
        if (numChildren > 0 )
        {
            identifier = jjtGetChild(numChildren - 1).getFirstToken().image;

			DEBUG.P("identifier2="+identifier);
        }

		DEBUG.P("identifier3="+identifier);
		...
		DEBUG.P("logOnNull="+logOnNull);
		DEBUG.P("strictRef="+strictRef);
		DEBUG.P("toStringNullCheck="+toStringNullCheck);
		...
		DEBUG.P("(strictRef && numChildren == 0)="+(strictRef && numChildren == 0));
        ...
        throws MethodInvocationException
    {
		...
		//如果找不到变理并且又没有在前端报错，那么尝试一下后台有没有注册事件监听器，
		//有的话就通知这样事件监听器有个变量找不到
            ...
            MethodInvocationException
    {
        ...
        if (value != null)
        {
			DEBUG.P("value.class="+value.getClass().getName());
        ...
        else
        {
			...
			DEBUG.P("context.getAllowRendering()="+context.getAllowRendering());
            if (context.getAllowRendering())
            {
                writer.write(escPrefix);
                writer.write(morePrefix);
    ...
    private String getRoot()
    {
		...
		DEBUG.P("slashbang="+slashbang);
		//如: $\!list.get(0).toString()
        if (slashbang != -1)
        {
            /*
     ...
     * @throws MethodInvocationException
     */
	//直接有context取出variable的值，相当于map.get(variable)
    public Object getVariableValue(Context context, String variable) throws MethodInvocationException
    {
		try {//我加上的
		DEBUG.P(this,"getVariableValue(2)");

		DEBUG.P("variable="+variable);
		...
		DEBUG.P("strictRef="+strictRef);
		//如果runtime.references.strict属性设成了true，但是找不到变量时有报错。
		...
		DEBUG.P(0,this,"getVariableValue(2)");
		}
    }
