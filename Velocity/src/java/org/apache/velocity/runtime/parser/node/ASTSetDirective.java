...
public class ASTSetDirective extends SimpleNode
{
    ...
    throws TemplateInitException
    {
		try {//我加上的
		DEBUG.P(this,"init(2)");
		DEBUG.P("isInitialized="+isInitialized);
        ...
        if (!isInitialized)
        {
			...
			DEBUG.P("allowNull="+allowNull);
			DEBUG.P("strictRef="+strictRef);

			DEBUG.P("left.getFirstToken().image="+left.getFirstToken().image);
            
            /*
             *  grab this now.  No need to redo each time
             */
            leftReference = left.getFirstToken().image.substring(1); //去掉$字符

			DEBUG.P("leftReference="+leftReference);
        ...
        throws IOException, MethodInvocationException
    {
        ...
        if ( value == null && !strictRef)
        {
			...
			DEBUG.P("rightReference="+rightReference);
        ...
        else
        {
			...
			DEBUG.P("leftReference="+leftReference);
			DEBUG.P("left.jjtGetNumChildren()="+left.jjtGetNumChildren());
