...
public class TreeInfo {
    private static my.Debug DEBUG=new my.Debug(my.Debug.TreeInfo);//我加上的
     ...
     */
    public Name operatorName(int tag) { //这里假定JCTree.POS<=tag<=JCTree.MOD
	...
	*/
    ...
    //DEBUG.P("skipParens(tree)(前):JCTree tree.getKind="+tree.getKind());
    
	tree = skipParens(tree);//取括号表达式中的表达式
	
	//DEBUG.P("skipParens(tree)(后):JCTree tree.getKind="+tree.getKind());
	...
	case JCTree.IDENT:
	    return ((JCIdent) tree).name;
	case JCTree.SELECT:
		//注意JCFieldAccess的内部结构，详情参考Parser.qualident()
		//sname总是最前面的IDENT
	...
	//}finally {
	//	DEBUG.P(0,TreeInfo.class,"fullName(1)");
	//}
    }
    ...
    public static final int
		...
		bitandPrec = 8,   // &
		eqPrec = 9,       // == !=
		ordPrec = 10,     // <  >  <=  >=  instanceof
		shiftPrec = 11,   // <<  >>  >>>
		addPrec = 12,     // +  -
		mulPrec = 13,     // *  /  %
		prefixPrec = 14,  // +  -  ! ~  ++_  --_   (一元运算符)
    ...
    public static int opPrec(int op) {
		...
		case JCTree.TYPETEST: return ordPrec; //instanceof运算符和比较运算符的优先级一样
