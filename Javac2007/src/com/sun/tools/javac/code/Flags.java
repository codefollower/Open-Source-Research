 ...
 */
@Version("@(#)Flags.java	1.47 07/03/21")
public class Flags {
    ...
    public static String toString(long flags) {
        ...
        //buf.append("0x"+Long.toHexString(flags)+" ");//我加上的
		...
		if ((flags&BLOCK) != 0) buf.append("block ");
        ...
        if ((flags&GENERATEDCONSTR) != 0) buf.append("generatedconstr ");
        if ((flags&HYPOTHETICAL) != 0) buf.append("hypothetical ");
        if ((flags&PROPRIETARY) != 0) buf.append("proprietary ");
        if(flags!=0 && buf.length()==0) buf.append(flags);
        //---------------------------------------------------------------
    ...
    // for example.
    public static final int ACC_SUPER    = 0x0020;//覆盖SYNCHRONIZED
    public static final int ACC_BRIDGE   = 0x0040;//覆盖VOLATILE
    public static final int ACC_VARARGS  = 0x0080;//覆盖TRANSIENT
