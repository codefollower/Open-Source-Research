...
@Version("@(#)ClassFile.java	1.31 07/03/21")
public class ClassFile {
    ...
    public static String myTAG(int tag) {
    	...
    	if(tag==CONSTANT_Integer) return "CONSTANT_Integer ("+CONSTANT_Integer+")";
    	if(tag==CONSTANT_Float) return "CONSTANT_Float ("+CONSTANT_Float+")";
    	if(tag==CONSTANT_Long) return "CONSTANT_Long ("+CONSTANT_Long+")";
    	if(tag==CONSTANT_Double) return "CONSTANT_Double ("+CONSTANT_Double+")";
    	if(tag==CONSTANT_Class ) return "CONSTANT_Class ("+CONSTANT_Class +")";
    	if(tag==CONSTANT_String) return "CONSTANT_String ("+CONSTANT_String+")";
    	if(tag==CONSTANT_Fieldref) return "CONSTANT_Fieldref ("+CONSTANT_Fieldref+")";
    	if(tag==CONSTANT_Methodref) return "CONSTANT_Methodref ("+CONSTANT_Methodref+")";
    	if(tag==CONSTANT_InterfaceMethodref) return "CONSTANT_InterfaceMethodref ("+CONSTANT_InterfaceMethodref+")";
    	if(tag==CONSTANT_NameandType) return "CONSTANT_NameandType ("+CONSTANT_NameandType+")";
    	return ""+tag;
    }
 ...
 ***********************************************************************/
     ...
     *  converting '/' to '.'.
     */
    //这里是指把class文件中的内容读取到内存中的表示形式(在ClassReader类中有应用)
     ...
     *  converting '.' to '/'.
     */
    //这里是指在生成的class文件中的表示形式(在ClassWriter类中有应用)
