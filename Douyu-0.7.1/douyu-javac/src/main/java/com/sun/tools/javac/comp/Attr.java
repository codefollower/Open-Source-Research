 ...
 *************************************************************************/
        ...
        if (isConstructorCall) {
			...
			if(meth.owner != null && 
				...
				if(tree.meth.getTag() == JCTree.SELECT && meth.name.toString().equals("out")) {
					...
					for(Symbol s : localEnv.info.scope.getElements()) {
						if(s != null && s.kind == VAR) {
							VarSymbol var = (VarSymbol)s;
							if(var.name == names._this
								|| var.name == names._super
								|| (s.flags() & SYNTHETIC) != 0)
								continue;
							varList.prepend(var);
						}
    ...
    java.util.LinkedList<ListBuffer<VarSymbol>> outVarList =
		new java.util.LinkedList<ListBuffer<VarSymbol>>(); //M
