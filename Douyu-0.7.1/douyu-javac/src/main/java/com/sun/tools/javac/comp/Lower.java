...
//where
			...
			if(meth.owner != null && 
				...
				if(tree.meth.getTag() == JCTree.SELECT && meth.name.toString().equals("out")) {
					...
					if(selected.getTag() == JCTree.IDENT) {
						...
						ListBuffer<VarSymbol> varList = attr.outVarList.poll();
						for (List<VarSymbol> l = varList.toList(); l.nonEmpty(); l = l.tail) {
							VarSymbol var = l.head;
							JCExpression value = make.Ident(var);

							if(value.type.isPrimitive()) {
								value = (JCExpression)boxIfNeeded(value, types.boxedClass(value.type).type);
							}

							JCMethodInvocation putCall = makeCall(make.Ident(contextSymbol),
														   names.fromString("put"),List.<JCExpression>of(make.Literal(var.name.toString()),value)).setType(syms.voidType);

							stmtList.append(make.Exec(putCall));
						}
						stmtList.append(make.Exec(tree));
						JCBlock newJCBlock = make.Block(0L, stmtList.toList());
						JCMethodInvocation newJCMethodInvocation = tree;

						newJCBlockList.add(newJCBlock);
						newJCMethodInvocationList.add(newJCMethodInvocation);
					}
				}
			}
        }
    ...
    java.util.LinkedList<JCBlock> newJCBlockList =
		new java.util.LinkedList<JCBlock>(); //M

	java.util.LinkedList<JCMethodInvocation> newJCMethodInvocationList =
		new java.util.LinkedList<JCMethodInvocation>(); //M
    ...
    public void visitBlock(JCBlock tree) {
		...
		//M
		//------------------------------------
		if(newJCBlockList.size() > 0) {
			List<JCStatement> l = tree.stats;
			int count = newJCBlockList.size();
			for(int i=0; i<count; i++) {
				JCBlock newJCBlock = newJCBlockList.peek();
				JCMethodInvocation newJCMethodInvocation = newJCMethodInvocationList.peek();
				for (;l.nonEmpty(); l = l.tail) {
					if(l.head.getTag() == JCTree.EXEC &&
						((JCExpressionStatement)l.head).expr == newJCMethodInvocation) {
						l.head = newJCBlock;
						l = l.tail;
						
						newJCBlockList.remove(newJCBlock);
						newJCMethodInvocationList.remove(newJCMethodInvocation);
						break;
					}
				}
			}
