...
import com.codefollower.douyu.netty.channel.nio.SocketSendBufferPool.SendBuffer;
import com.codefollower.douyu.netty.util.internal.DeadLockProofWorker;
import com.codefollower.douyu.netty.util.internal.LinkedTransferQueue;
...
class NioWorker implements Runnable {
    ...
    void register(NioAcceptedSocketChannel channel) {
        Runnable registerTask = new RegisterTask(channel);
        Selector selector;
    ...
    private boolean read(SelectionKey k) {
        ...
        final NioAcceptedSocketChannel channel = (NioAcceptedSocketChannel) k.attachment();
    ...
    void writeFromUserCode(final NioAcceptedSocketChannel channel) {
    ...
    void writeFromTaskLoop(final NioAcceptedSocketChannel ch) {
    ...
    private boolean scheduleWriteIfNecessary(final NioAcceptedSocketChannel channel) {
    ...
    private void write0(NioAcceptedSocketChannel channel) {
        boolean open = true;
    ...
    private void setOpWrite(NioAcceptedSocketChannel channel) {
        Selector selector = this.selector;
    ...
    private void clearOpWrite(NioAcceptedSocketChannel channel) {
        Selector selector = this.selector;
        SelectionKey key = channel.socket.keyFor(selector);
        if (key == null) {
            return;
    ...
    private void cleanUpWriteBuffer(NioAcceptedSocketChannel channel) {
    ...
    void setInterestOps(
            NioAcceptedSocketChannel channel, ChannelFuture future, int interestOps) {
    ...
    private final class RegisterTask implements Runnable {
        private final NioAcceptedSocketChannel channel;
