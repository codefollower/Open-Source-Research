...
import com.codefollower.douyu.http.ssl.jsse.JSSEImplementation;
...
public abstract class SSLImplementation {
	private static final InternalLogger logger = InternalLoggerFactory.getInstance(SSLImplementation.class);
        ...
        for (int i = 0; i < implementations.length; i++) {
            try {
                ...
                if (logger.isDebugEnabled())
                    logger.debug("Error creating " + implementations[i], e);
            }
        }
    ...
    public abstract SSLUtil getSSLUtil(SSLConfig config);
}
