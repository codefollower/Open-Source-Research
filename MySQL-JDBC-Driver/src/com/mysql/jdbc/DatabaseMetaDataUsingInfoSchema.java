...
public class DatabaseMetaDataUsingInfoSchema extends DatabaseMetaData {
	...
	private ResultSet executeMetadataQuery(java.sql.PreparedStatement pStmt) throws SQLException {
	...
	public java.sql.ResultSet getColumnPrivileges(String catalog, String schema, String table, String columnNamePattern)
		...
		String sql = "SELECT TABLE_SCHEMA AS TABLE_CAT, NULL AS TABLE_SCHEM, TABLE_NAME,"
				...
				+ "INFORMATION_SCHEMA.COLUMN_PRIVILEGES WHERE " + "TABLE_SCHEMA LIKE ? AND "
				+ "TABLE_NAME =? AND COLUMN_NAME LIKE ? ORDER BY " + "COLUMN_NAME, PRIVILEGE_TYPE";
		...
		try {
			...
			((com.mysql.jdbc.ResultSetInternalMethods) rs).redefineFieldsForDBMD(new Field[] {
					new Field("", "TABLE_CAT", Types.CHAR, 64), new Field("", "TABLE_SCHEM", Types.CHAR, 1),
					new Field("", "TABLE_NAME", Types.CHAR, 64), new Field("", "COLUMN_NAME", Types.CHAR, 64),
					new Field("", "GRANTOR", Types.CHAR, 77), new Field("", "GRANTEE", Types.CHAR, 77),
					new Field("", "PRIVILEGE", Types.CHAR, 64), new Field("", "IS_GRANTABLE", Types.CHAR, 3) });
	...
	public ResultSet getColumns(String catalog, String schemaPattern, String tableName, String columnNamePattern)
		...
		StringBuffer sqlBuf = new StringBuffer("SELECT " + "TABLE_SCHEMA AS TABLE_CAT, " + "NULL AS TABLE_SCHEM,"
		...
		if (conn.getCapitalizeTypeNames()) {
			sqlBuf
					.append("UPPER(CASE WHEN LOCATE('unsigned', COLUMN_TYPE) != 0 AND LOCATE('unsigned', DATA_TYPE) = 0 THEN CONCAT(DATA_TYPE, ' unsigned') ELSE DATA_TYPE END) AS TYPE_NAME,");
		} else {
			sqlBuf
					.append("CASE WHEN LOCATE('unsigned', COLUMN_TYPE) != 0 AND LOCATE('unsigned', DATA_TYPE) = 0 THEN CONCAT(DATA_TYPE, ' unsigned') ELSE DATA_TYPE END AS TYPE_NAME,");
		}
	...
	public java.sql.ResultSet getCrossReference(String primaryCatalog, String primarySchema, String primaryTable,
		...
		String sql = "SELECT " + "A.REFERENCED_TABLE_SCHEMA AS PKTABLE_CAT," + "NULL AS PKTABLE_SCHEM,"
	...
	private String generateOptionalRefContraintsJoin() {
		return ((this.hasReferentialConstraintsView) ? "JOIN " + "INFORMATION_SCHEMA.REFERENTIAL_CONSTRAINTS R "
	...
	private String generateDeleteRuleClause() {
		return ((this.hasReferentialConstraintsView) ? "CASE WHEN R.DELETE_RULE='CASCADE' THEN "
				+ String.valueOf(importedKeyCascade) + " WHEN R.DELETE_RULE='SET NULL' THEN "
				+ String.valueOf(importedKeySetNull) + " WHEN R.DELETE_RULE='SET DEFAULT' THEN "
				+ String.valueOf(importedKeySetDefault) + " WHEN R.DELETE_RULE='RESTRICT' THEN "
				+ String.valueOf(importedKeyRestrict) + " WHEN R.DELETE_RULE='NO ACTION' THEN "
	...
	private String generateUpdateRuleClause() {
		return ((this.hasReferentialConstraintsView) ? "CASE WHEN R.UPDATE_RULE='CASCADE' THEN "
				+ String.valueOf(importedKeyCascade) + " WHEN R.UPDATE_RULE='SET NULL' THEN "
				+ String.valueOf(importedKeySetNull) + " WHEN R.UPDATE_RULE='SET DEFAULT' THEN "
				+ String.valueOf(importedKeySetDefault) + " WHEN R.UPDATE_RULE='RESTRICT' THEN "
				+ String.valueOf(importedKeyRestrict) + " WHEN R.UPDATE_RULE='NO ACTION' THEN "
				+ String.valueOf(importedKeyNoAction) + " ELSE " + String.valueOf(importedKeyNoAction) + " END "
				: String.valueOf(importedKeyRestrict));
	}
	...
	public java.sql.ResultSet getImportedKeys(String catalog, String schema, String table) throws SQLException {
		...
		String sql = "SELECT " + "A.REFERENCED_TABLE_SCHEMA AS PKTABLE_CAT," + "NULL AS PKTABLE_SCHEM,"
				...
				+ "ORDER BY "
				+ "A.REFERENCED_TABLE_SCHEMA, A.REFERENCED_TABLE_NAME, " + "A.ORDINAL_POSITION";
	...
	public ResultSet getIndexInfo(String catalog, String schema, String table, boolean unique, boolean approximate)
			throws SQLException {
		StringBuffer sqlBuf = new StringBuffer("SELECT " + "TABLE_SCHEMA AS TABLE_CAT, " + "NULL AS TABLE_SCHEM,"
				+ "TABLE_NAME," + "NON_UNIQUE," + "TABLE_SCHEMA AS INDEX_QUALIFIER," + "INDEX_NAME," + tableIndexOther
				+ " AS TYPE," + "SEQ_IN_INDEX AS ORDINAL_POSITION," + "COLUMN_NAME," + "COLLATION AS ASC_OR_DESC,"
	...
	public java.sql.ResultSet getPrimaryKeys(String catalog, String schema, String table) throws SQLException {
		...
		try {
			...
			ResultSet rs = executeMetadataQuery(pStmt);
			((com.mysql.jdbc.ResultSetInternalMethods) rs).redefineFieldsForDBMD(new Field[] {
					new Field("", "TABLE_CAT", Types.CHAR, 255), new Field("", "TABLE_SCHEM", Types.CHAR, 0),
					new Field("", "TABLE_NAME", Types.CHAR, 255), new Field("", "COLUMN_NAME", Types.CHAR, 32),
	...
	public ResultSet getProcedures(String catalog, String schemaPattern, String procedureNamePattern)
		...
		String sql = "SELECT ROUTINE_SCHEMA AS PROCEDURE_CAT, " + "NULL AS PROCEDURE_SCHEM, "
				+ "ROUTINE_NAME AS PROCEDURE_NAME, " + "NULL AS RESERVED_1, " + "NULL AS RESERVED_2, "
				+ "NULL AS RESERVED_3, " + "ROUTINE_COMMENT AS REMARKS, "
				+ "CASE WHEN ROUTINE_TYPE = 'PROCEDURE' THEN " + procedureNoResult
				+ " WHEN ROUTINE_TYPE='FUNCTION' THEN " + procedureReturnsResult + " ELSE " + procedureResultUnknown
				+ " END AS PROCEDURE_TYPE " + "FROM INFORMATION_SCHEMA.ROUTINES WHERE "
				+ "ROUTINE_SCHEMA LIKE ? AND ROUTINE_NAME LIKE ? " + "ORDER BY ROUTINE_SCHEMA, ROUTINE_NAME";
		...
		try {
			...
			ResultSet rs = executeMetadataQuery(pStmt);
			((com.mysql.jdbc.ResultSetInternalMethods) rs).redefineFieldsForDBMD(new Field[] {
					new Field("", "PROCEDURE_CAT", Types.CHAR, 0), new Field("", "PROCEDURE_SCHEM", Types.CHAR, 0),
					new Field("", "PROCEDURE_NAME", Types.CHAR, 0), new Field("", "reserved1", Types.CHAR, 0),
					new Field("", "reserved2", Types.CHAR, 0), new Field("", "reserved3", Types.CHAR, 0),
					new Field("", "REMARKS", Types.CHAR, 0), new Field("", "PROCEDURE_TYPE", Types.SMALLINT, 0) });
	...
	public ResultSet getFunctionColumns(String catalog, String schemaPattern, String functionNamePattern,
		...
		StringBuffer sqlBuf = new StringBuffer("SELECT SPECIFIC_SCHEMA AS FUNCTION_CAT, "
				+ "NULL AS `FUNCTION_SCHEM`, " + "SPECIFIC_NAME AS `FUNCTION_NAME`, "
				+ "PARAMETER_NAME AS `COLUMN_NAME`, " + "CASE WHEN PARAMETER_MODE = 'IN' THEN " + 1 /* functionColumnIn */
		...
		if (conn.getCapitalizeTypeNames()) {
			sqlBuf
					.append("UPPER(CASE WHEN LOCATE('unsigned', DATA_TYPE) != 0 AND LOCATE('unsigned', DATA_TYPE) = 0 THEN CONCAT(DATA_TYPE, ' unsigned') ELSE DATA_TYPE END) AS `TYPE_NAME`,");
		} else {
			sqlBuf
					.append("CASE WHEN LOCATE('unsigned', DATA_TYPE) != 0 AND LOCATE('unsigned', DATA_TYPE) = 0 THEN CONCAT(DATA_TYPE, ' unsigned') ELSE DATA_TYPE END AS `TYPE_NAME`,");
		}
		...
		// SPECIFIC_NAME *
		sqlBuf
				.append(2 /* functionNullableUnknown */
						+ " AS `NULLABLE`, "
						+ " NULL AS `REMARKS`, "
	...
	public ResultSet getProcedureColumns(String catalog, String schemaPattern, String procedureNamePattern,
		...
		StringBuffer sqlBuf = new StringBuffer("SELECT SPECIFIC_SCHEMA AS PROCEDURE_CAT, "
				+ "NULL AS `PROCEDURE_SCHEM`, " + "SPECIFIC_NAME AS `PROCEDURE_NAME`, "
				+ "PARAMETER_NAME AS `COLUMN_NAME`, " + "CASE WHEN PARAMETER_MODE = 'IN' THEN " + procedureColumnIn
				+ " WHEN PARAMETER_MODE='OUT' THEN " + procedureColumnOut + " WHEN PARAMETER_MODE='INOUT' THEN "
				+ procedureColumnInOut + " WHEN ORDINAL_POSITION=0 THEN " + procedureColumnReturn + " ELSE "
				+ procedureColumnUnknown + " END AS `COLUMN_TYPE`, ");
		...
		sqlBuf.append("10 AS RADIX,");
		sqlBuf
				.append(procedureNullableUnknown
						+ " AS `NULLABLE`, "
						+ " NULL AS `REMARKS` "
						+ "FROM INFORMATION_SCHEMA.PARAMETERS WHERE "
						+ "SPECIFIC_SCHEMA LIKE ? AND SPECIFIC_NAME LIKE ? AND (PARAMETER_NAME LIKE ? OR PARAMETER_NAME IS NULL) "
	...
	public ResultSet getTables(String catalog, String schemaPattern, String tableNamePattern, String[] types)
		...
		String sql = "SELECT TABLE_SCHEMA AS TABLE_CAT, "
				...
				+ "TABLE_COMMENT AS REMARKS " + "FROM INFORMATION_SCHEMA.TABLES WHERE "
		...
		try {
			...
			((com.mysql.jdbc.ResultSetInternalMethods) rs).redefineFieldsForDBMD(new Field[] {
					new Field("", "TABLE_CAT", java.sql.Types.VARCHAR, (catalog == null) ? 0 : catalog.length()),
