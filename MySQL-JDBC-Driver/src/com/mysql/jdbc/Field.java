...
public class Field {
	...
	//original开头的是指as左边的值，没有original的是指as右边的值
	//如"select name, age as a from pet as b"，
	//originalColumnName=age,name=a,originalTableName=pet,tableName=b
	//没有as时，都一样，如name字段originalColumnName=name="name"

	/**
	 * Constructor used when communicating with 4.1 and newer servers
	 */
	Field(MySQLConnection conn, byte[] buffer, int databaseNameStart, int databaseNameLength, int tableNameStart,
			int tableNameLength, int originalTableNameStart, int originalTableNameLength, int nameStart,
			int nameLength, int originalColumnNameStart, int originalColumnNameLength, long length, int mysqlType,
			short colFlag, int colDecimals, int defaultValueStart, int defaultValueLength, int charsetIndex)
			throws SQLException {
		...
		if (!isNativeNumericType() && !isNativeDateTimeType()) {
			...
			if (this.connection.versionMeetsMinimum(4, 1, 0) && this.mysqlType == MysqlDefs.FIELD_TYPE_VAR_STRING
			...
			if (this.connection.versionMeetsMinimum(4, 1, 0) && this.mysqlType == MysqlDefs.FIELD_TYPE_STRING
					&& isBinary && this.charsetIndex == 63) {
				//
			...
			if (this.mysqlType == MysqlDefs.FIELD_TYPE_BIT) {
				...
				if (this.connection != null
						&& (this.connection.versionMeetsMinimum(5, 0, 21) || this.connection.versionMeetsMinimum(5, 1,
								10)) && this.length == 1) {
	 ...
	 */
	Field(MySQLConnection conn, byte[] buffer, int nameStart, int nameLength, int tableNameStart, int tableNameLength,
			int length, int mysqlType, short colFlag, int colDecimals) throws SQLException {
		this(conn, buffer, -1, -1, tableNameStart, tableNameLength, -1, -1, nameStart, nameLength, -1, -1, length,
				mysqlType, colFlag, colDecimals, -1, -1, NO_CHARSET_INFO);
	}
	...
	private void checkForImplicitTemporaryTable() {
		this.isImplicitTempTable = this.tableNameLength > 5 && this.buffer[tableNameStart] == (byte) '#'
				&& this.buffer[tableNameStart + 1] == (byte) 's' && this.buffer[tableNameStart + 2] == (byte) 'q'
				&& this.buffer[tableNameStart + 3] == (byte) 'l' && this.buffer[tableNameStart + 4] == (byte) '_';
	}
			...
			if (this.connection != null) {
				if (this.connection.versionMeetsMinimum(4, 1, 0)) {
					if (this.connection.getUseDynamicCharsetInfo()) {
						...
						if (csCatalogName != null && csCatalogName.length() != 0 && csTableName != null
								&& csTableName.length() != 0 && csColumnName != null && csColumnName.length() != 0) {
	...
	public String getFullOriginalName() throws SQLException {
		...
		if (this.fullName == null) {
			StringBuffer fullOriginalNameBuf = new StringBuffer(getOriginalTableName().length() + 1
	...
	public String getOriginalName() throws SQLException {
		if ((this.originalColumnName == null) && (this.originalColumnNameStart != -1)
	...
	public String getOriginalTableName() throws SQLException {
		if ((this.originalTableName == null) && (this.originalTableNameStart != -1)
				...
				if (encoding != null) {
					...
					if (converter != null) { // we have a converter
						...
						try {
							...
							throw new RuntimeException(Messages.getString("Field.12") + encoding //$NON-NLS-1$
	...
	boolean isReadOnly() throws SQLException {
		if (this.connection.versionMeetsMinimum(4, 1, 0)) {
			...
			return !(orgColumnName != null && orgColumnName.length() > 0 && orgTableName != null && orgTableName
					.length() > 0);
		}
