...
public class PreparedStatement extends com.mysql.jdbc.StatementImpl implements java.sql.PreparedStatement {
	private static my.Debug DEBUG=new my.Debug(my.Debug.PreparedStatement);//我加上的
	...
	//与ServerPreparedStatement的BatchedBindValues类似，
	//每一个BatchParams实例就代表一次addBatch()，
	//isNull、isStream这些数组类型的字段的每一个元素
	//就代表一个setXXX进来的参数的值是否为null，是否是InputStream这样的Stream
	class BatchParams {
		boolean[] isNull = null;
		...
		BatchParams(byte[][] strings, InputStream[] streams, boolean[] isStreamFlags, int[] lengths,
				boolean[] isNullFlags) {
			//
			// Make copies
			//
	...
	class ParseInfo {
		...
		//如:"select * from pet /* a middle comment */ where id=? and name=? or aga=10"
		//则分成三部份:
		//1)"select * from pet /* a middle comment */ where id="
		//2)" and name="
		//3)" or aga=10"
		//然后分别转换成字节数组
		byte[][] staticSql = null;
		 ...
		 * parts.
		 */
		ParseInfo(String sql, MySQLConnection conn, java.sql.DatabaseMetaData dbmd, String encoding,
				SingleByteCharsetConverter converter) throws SQLException {
			this(sql, conn, dbmd, encoding, converter, true);
		}

		public ParseInfo(String sql, MySQLConnection conn, java.sql.DatabaseMetaData dbmd, String encoding,
				SingleByteCharsetConverter converter, boolean buildRewriteInfo) throws SQLException {
			try {//我加上的
				DEBUG.P(this, "ParseInfo(6)");
				DEBUG.P("sql=" + sql);
				DEBUG.P("encoding=" + encoding);
				DEBUG.P("converter=" + converter);
				DEBUG.P("buildRewriteInfo=" + buildRewriteInfo);
			...
			try {
				...
				char quotedIdentifierChar = 0; //是"`"号

				DEBUG.P("locationOfOnDuplicateKeyUpdate=" + locationOfOnDuplicateKeyUpdate);
				DEBUG.P("quotedIdentifierString=" + quotedIdentifierString);
				...
				boolean inQuotes = false; //是否在单引号对或双引号对中
				char quoteChar = 0;
				boolean inQuotedId = false; //`号开启
				int lastParmEnd = 0;
				int i;
				...
				DEBUG.P("noBackslashEscapes=" + noBackslashEscapes);
				DEBUG.P("statementStartPos=" + statementStartPos);
				...
				for (i = statementStartPos; i < this.statementLength; ++i) {
					...
					//当!noBackslashEscapes为true时可以在sql中用\转义
				...
				for (i = 0; i < this.staticSql.length; i++) {
					...
					DEBUG.P("new String(asCharArray, begin, len)="+new String(asCharArray, begin, len));
					...
					if (this.foundLoadData) {
						...
						if (converter != null) {
							this.staticSql[i] = StringUtils.getBytes(sql, converter, encoding, connection
									.getServerCharacterEncoding(), begin, len, connection.parserKnowsUnicode(),
									getExceptionInterceptor());
						} else {
							String temp = new String(asCharArray, begin, len);

							this.staticSql[i] = StringUtils.getBytes(temp, encoding, connection
									.getServerCharacterEncoding(), connection.parserKnowsUnicode(), conn,
									getExceptionInterceptor());
						}
					}
				}
			...
			//这个与ON DUPLICATE KEY UPDATE相关

			if (buildRewriteInfo) {
				this.canRewriteAsMultiValueInsert = PreparedStatement.canRewrite(sql, this.isOnDuplicateKeyUpdate,
						this.locationOfOnDuplicateKeyUpdate, this.statementStartPos)
						&& !this.parametersInDuplicateKeyClause;

				DEBUG.P("canRewriteAsMultiValueInsert=" + canRewriteAsMultiValueInsert);
				DEBUG.P("conn.getRewriteBatchedStatements()=" + conn.getRewriteBatchedStatements());
		...
		private void buildRewriteBatchedParams(String sql, MySQLConnection conn, DatabaseMetaData metadata,
				String encoding, SingleByteCharsetConverter converter) throws SQLException {
			...
			if (odkuClause != null && odkuClause.length() > 0) {
				this.batchODKUClause = new ParseInfo("," + this.valuesClause + " " + odkuClause, conn, metadata,
						encoding, converter, false);
			}
		}

		private String extractValuesClause(String sql) throws SQLException {
			String quoteCharStr = connection.getMetaData().getIdentifierQuoteString();
			...
			while (indexOfValues == -1) {
				if (quoteCharStr.length() > 0) {
					indexOfValues = StringUtils.indexOfIgnoreCaseRespectQuotes(valuesSearchStart, originalSql,
							"VALUES", quoteCharStr.charAt(0), false);
				} else {
		...
		synchronized ParseInfo getParseInfoForBatch(int numBatch) {
			...
			ParseInfo batchParseInfo = new ParseInfo(apv.getStaticSqlStrings(), this.firstStmtChar,
					this.foundLimitClause, this.foundLoadData, this.isOnDuplicateKeyUpdate,
		...
		private ParseInfo(byte[][] staticSql, char firstStmtChar, boolean foundLimitClause, boolean foundLoadData,
				boolean isOnDuplicateKeyUpdate, int locationOfOnDuplicateKeyUpdate, int statementLength,
	...
	private final static byte[] HEX_DIGITS = new byte[] { (byte) '0', (byte) '1', (byte) '2', (byte) '3', (byte) '4',
	...
	protected static PreparedStatement getInstance(MySQLConnection conn, String catalog) throws SQLException {
		...
		return (PreparedStatement) Util.handleNewInstance(JDBC_4_PSTMT_2_ARG_CTOR, new Object[] { conn, catalog }, conn
				.getExceptionInterceptor());
	}
	...
	protected static PreparedStatement getInstance(MySQLConnection conn, String sql, String catalog)
		...
		return (PreparedStatement) Util.handleNewInstance(JDBC_4_PSTMT_3_ARG_CTOR, new Object[] { conn, sql, catalog },
	...
	protected static PreparedStatement getInstance(MySQLConnection conn, String sql, String catalog,
		...
		return (PreparedStatement) Util.handleNewInstance(JDBC_4_PSTMT_4_ARG_CTOR, new Object[] { conn, sql, catalog,
	...
	public PreparedStatement(MySQLConnection conn, String sql, String catalog) throws SQLException {
		...
		try {//我加上的
		DEBUG.P(this,"PreparedStatement(3)");
		...
		DEBUG.P("useTrueBoolean="+useTrueBoolean);
		DEBUG.P("compensateForOnDuplicateKeyUpdate="+compensateForOnDuplicateKeyUpdate);
		DEBUG.P("conn.getRequiresEscapingEncoder()="+conn.getRequiresEscapingEncoder());
		DEBUG.P("(!this.connection.useAnsiQuotedIdentifiers())="+(!this.connection.useAnsiQuotedIdentifiers()));

		}finally{//我加上的
		DEBUG.P(0,this,"PreparedStatement(3)");
		}
	}
	...
	public void addBatch() throws SQLException {
		//batchedArgs在超类StatementImpl中定义，超类中用来放sql，这里用来放批量参数
		...
		this.batchedArgs.add(new BatchParams(this.parameterValues, this.parameterStreams, this.isStream,
				this.streamLengths, this.isNull));
	}
	...
	protected String asSql(boolean quoteStreamsAndUnknowns) throws SQLException {
		...
		try {
			...
			if (this.charEncoding != null) {
				buf.append(new String(this.staticSqlStrings[this.parameterCount + getParameterIndexOffset()],
						this.charEncoding));
			} else {
	...
	public boolean execute() throws SQLException {
		...
		synchronized (locallyScopedConn.getMutex()) {
			...
			if (locallyScopedConn.useMaxRows()) {
				...
				rs = executeInternal(rowLimit, sendPacket, doStreaming, (this.firstCharOfStmt == 'S'),
						metadataFromCache, false);
			} else {
				rs = executeInternal(-1, sendPacket, doStreaming, (this.firstCharOfStmt == 'S'), metadataFromCache,
						false);
			}
			...
			try {
				...
				if (!this.batchHasPlainStatements && this.connection.getRewriteBatchedStatements()) {
					...
					if (this.connection.versionMeetsMinimum(4, 1, 0) && !this.batchHasPlainStatements
							&& this.batchedArgs != null && this.batchedArgs.size() > 3 /* cost of option setting rt-wise */) {
		...
		synchronized (this.connection.getMutex()) {
			...
			try {
				...
				try {
					...
					if (this.retrieveGeneratedKeys) {
						batchedStatement = locallyScopedConn.prepareStatement(
								generateMultiStatementForBatch(numValuesPerBatch), RETURN_GENERATED_KEYS);
					} else {
					...
					for (int i = 0; i < numberArgsToExecute; i++) {
						if (i != 0 && i % numValuesPerBatch == 0) {
							...
							updateCountCounter = processMultiCountsAndKeys((StatementImpl) batchedStatement,
									updateCountCounter, updateCounts);

							batchedStatement.clearParameters();
							batchedParamIndex = 1;
						}

						batchedParamIndex = setOneBatchedParameterSet(batchedStatement, batchedParamIndex,
				...
				try {
					...
					if (sqlEx != null) {
						throw new java.sql.BatchUpdateException(sqlEx.getMessage(), sqlEx.getSQLState(), sqlEx
		...
		try {
			try {
				...
				for (int i = 0; i < numberArgsToExecute; i++) {
					...
					batchedParamIndex = setOneBatchedParameterSet(batchedStatement, batchedParamIndex, this.batchedArgs
							.get(batchCounter++));
				}
			...
			try {
				...
				for (batchCommandIndex = 0; batchCommandIndex < nbrCommands; batchCommandIndex++) {
					...
					if (arg instanceof String) {
						...
						try {
							updateCounts[batchCommandIndex] = executeUpdate(paramArg.parameterStrings,
									paramArg.parameterStreams, paramArg.isStream, paramArg.streamLengths,
									paramArg.isNull, true);
							...
							if (this.continueBatchOnError && !(ex instanceof MySQLTimeoutException)
								...
								throw new java.sql.BatchUpdateException(ex.getMessage(), ex.getSQLState(), ex
				...
				try {
					...
					throw new java.sql.BatchUpdateException(connectionClosedEx.getMessage(), connectionClosedEx
							.getSQLState(), connectionClosedEx.getErrorCode(), newUpdateCounts);
				}
	 ...
	 *             if an error occurs.
	 */
	protected ResultSetInternalMethods executeInternal(int maxRowsToRetrieve, Buffer sendPacket,
			boolean createStreamingResultSet, boolean queryIsSelectOnly, Field[] metadataFromCache, boolean isBatch)
			throws SQLException {
		try {
			...
			try {
				...
				rs = locallyScopedConnection.execSQL(this, null, maxRowsToRetrieve, sendPacket, this.resultSetType,
						this.resultSetConcurrency, createStreamingResultSet, this.currentCatalog, metadataFromCache,
						isBatch);
	...
	public java.sql.ResultSet executeQuery() throws SQLException {
		...
		synchronized (locallyScopedConn.getMutex()) {
			...
			if (locallyScopedConn.useMaxRows()) {
				...
				// a query, and network traffic).
				if (this.hasLimitClause) {
					this.results = executeInternal(this.maxRows, sendPacket, createStreamingResultSet(), true,
							metadataFromCache, false);
				} else {
					if (this.maxRows <= 0) {
						executeSimpleNonQuery(locallyScopedConn, "SET OPTION SQL_SELECT_LIMIT=DEFAULT");
		...
		DEBUG.P("lastInsertId="+lastInsertId);

		return this.results;

		}finally{//我加上的
		DEBUG.P(0,this,"executeQuery()");
		}
	}
	...
	protected int executeUpdate(boolean clearBatchedGeneratedKeysAndWarnings, boolean isBatch) throws SQLException {
		...
		return executeUpdate(this.parameterValues, this.parameterStreams, this.isStream, this.streamLengths,
				this.isNull, isBatch);
	}
	 ...
	 *             if a database error occurs
	 */
	protected int executeUpdate(byte[][] batchedParameterStrings, InputStream[] batchedParameterStreams,
			boolean[] batchedIsStream, int[] batchedStreamLengths, boolean[] batchedIsNull, boolean isReallyBatch)
			throws SQLException {
		...
		if ((this.firstCharOfStmt == 'S') && isSelectQuery()) { //$NON-NLS-1$
		...
		synchronized (locallyScopedConn.getMutex()) {
			Buffer sendPacket = fillSendPacket(batchedParameterStrings, batchedParameterStreams, batchedIsStream,
	 ...
	 *             if an error occurs.
	 */
	protected Buffer fillSendPacket(byte[][] batchedParameterStrings, InputStream[] batchedParameterStreams,
		...
		if (statementComment != null) {
			if (this.charConverter != null) {
				...
				commentAsBytes = StringUtils.getBytes(statementComment, this.charConverter, this.charEncoding,
	...
	protected PreparedStatement prepareBatchedInsertSQL(MySQLConnection localConn, int numBatches) throws SQLException {
		PreparedStatement pstmt = new PreparedStatement(localConn, "Rewritten batch of: " + this.originalSql,
				this.currentCatalog, this.parseInfo.getParseInfoForBatch(numBatches));
		...
		if (this.pstmtResultMetaData == null) {
			try {
				...
				if (hadResults) {
					...
					this.pstmtResultMetaData = new ResultSetMetaData(new Field[0], this.connection
							.getUseOldAliasMetadataBehavior(), getExceptionInterceptor());
				}
			} finally {
	...
	protected boolean isSelectQuery() {
		return StringUtils.startsWithIgnoreCaseAndWs(StringUtils.stripComments(this.originalSql, "'\"", "'\"", true,
				false, true, true), "SELECT");
	}
	...
	private final char getSuccessor(char c, int n) {
		return ((c == 'y') && (n == 2)) ? 'X' : (((c == 'y') && (n < 4)) ? 'y' : ((c == 'y') ? 'M'
				: (((c == 'M') && (n == 2)) ? 'Y' : (((c == 'M') && (n < 3)) ? 'M' : ((c == 'M') ? 'd'
						: (((c == 'd') && (n < 2)) ? 'd' : ((c == 'd') ? 'H' : (((c == 'H') && (n < 2)) ? 'H'
								: ((c == 'H') ? 'm' : (((c == 'm') && (n < 2)) ? 'm' : ((c == 'm') ? 's'
										: (((c == 's') && (n < 2)) ? 's' : 'W'))))))))))));
	}
	...
	private void initializeFromParseInfo() throws SQLException {
		...
		//因为?后总会有另一个子串，不管是empty串还是非empty串
		...
		if (this.useUsageAdvisor) {
			if (this.numberOfExecutions <= 1) {
				...
				this.eventSink.consumeEvent(new ProfilerEvent(ProfilerEvent.TYPE_WARN, "", this.currentCatalog, //$NON-NLS-1$
						this.connectionId, this.getId(), -1, System.currentTimeMillis(), 0, Constants.MILLIS_I18N,
	 ...
	 *                if a database access error occurs
	 */
	//这个方法是在1.6前就有的，在1.6中加了下面两个新方法:
	//setAsciiStream(int parameterIndex,InputStream x)
	//setAsciiStream(int parameterIndex, InputStream x, long length)
	//上面这两个新方法都是调用这个方法，前者把length变成-1，后者强制把long转成int
	public void setAsciiStream(int parameterIndex, InputStream x, int length) throws SQLException {
		if (x == null) {
			//指定这个java.sql.Types.VARCHAR其实没用到，setNull内部还是用Types.NULL
			setNull(parameterIndex, java.sql.Types.VARCHAR);
		} else {
			//Ascii码可以用一个字节表示，所以可以直接用setBinaryStream来处理setAsciiStream
	 ...
	 *                if a database access error occurs
	 */
	//BigDecimal转换成String(再转成字节数组)之后发到数据库
	public void setBigDecimal(int parameterIndex, BigDecimal x) throws SQLException {
		try {//我加上的
		DEBUG.P(this,"setBigDecimal(2)");
		DEBUG.P("parameterIndex="+parameterIndex);
		DEBUG.P("x="+x);
		DEBUG.P("x.toPlainString()="+x.toPlainString()); //不带e(指数)的形式
		DEBUG.P("12E3="+StringUtils.fixDecimalExponent("12E3"));

		if (x == null) {
			setNull(parameterIndex, java.sql.Types.DECIMAL);
		} else {
			//如果BigDecimal存在toPlainString()方法就用toPlainString()，否则用toString()
			//如果e后没有正号，fixDecimalExponent会加上，如:12E3变成12E+3
		...
		DEBUG.P(0,this,"setBigDecimal(2)");
		}
	}
	...
	public void setBinaryStream(int parameterIndex, InputStream x, int length) throws SQLException {
		try {//我加上的
		DEBUG.P(this,"setBinaryStream(3)");
		DEBUG.P("parameterIndex="+parameterIndex);
		DEBUG.P("x="+x);
		...
		if (x == null) {
			...
			//CallableStatement覆盖了getParameterIndexOffset方法，
			//如果是调用一个存储函数{? call ...}，则返回-1
			int parameterIndexOffset = getParameterIndexOffset();

			DEBUG.P("parameterIndexOffset="+parameterIndexOffset);
			DEBUG.P("this.staticSqlStrings.length="+this.staticSqlStrings.length);
			
			//(parameterIndex > this.staticSqlStrings.length)是一个BUG，因为staticSqlStrings.length是?号个数+1
			//比如:
			//ps = conn.prepareStatement("select * from pet where id=?");
			//ps.setBinaryStream(2, bais);
			//这个if语句没法判断出来，最后导致ArrayIndexOutOfBoundsException
			//正确应是(parameterIndex > this.staticSqlStrings.length-1)
			...
			.createSQLException(
					...
					SQLError.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
			*/
			if ((parameterIndex < 1) || (parameterIndex > this.staticSqlStrings.length-1)) {
				throw SQLError
						.createSQLException(
								Messages.getString("PreparedStatement.2") //$NON-NLS-1$
										+ parameterIndex
										+ Messages.getString("PreparedStatement.3") + (this.staticSqlStrings.length-1) + Messages.getString("PreparedStatement.4"), //$NON-NLS-1$ //$NON-NLS-2$
								SQLError.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
		...
		DEBUG.P(0,this,"setBinaryStream(3)");
		}
	}
	...
	public void setCharacterStream(int parameterIndex, java.io.Reader reader, int length) throws SQLException {
		try {
				...
				if (useLength && (length != -1)) {
					...
					if (forcedEncoding == null) {
						...
						try {
							...
							throw SQLError.createSQLException("Unsupported character encoding " + forcedEncoding,
									SQLError.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
						}
					}
				} else {
			...
			throw SQLError.createSQLException(ioEx.toString(), SQLError.SQL_STATE_GENERAL_ERROR,
					getExceptionInterceptor());
		}
	}
	...
	public void setDouble(int parameterIndex, double x) throws SQLException {
		...
		if (!this.connection.getAllowNanAndInf()
			...
			throw SQLError.createSQLException("'" + x + "' is not a valid numeric or approximate numeric value",
					SQLError.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
	...
	protected final void setInternal(int paramIndex, String val) throws SQLException {
		...
		if (this.charConverter != null) {
			...
			parameterAsBytes = StringUtils.getBytes(val, this.charConverter, this.charEncoding, this.connection
					.getServerCharacterEncoding(), this.connection.parserKnowsUnicode(), getExceptionInterceptor());
		}

		setInternal(paramIndex, parameterAsBytes);
		
		}finally{//我加上的
		DEBUG.P(0,this,"setInternal(2)");
		}
	}
	...
	private void setNumericObject(int parameterIndex, Object parameterObj, int targetSqlType, int scale)
		...
		if (parameterObj instanceof Boolean) {
			parameterAsNum = ((Boolean) parameterObj).booleanValue() ? Constants.integerValueOf(1) : Constants
					.integerValueOf(0);
			...
			if (parameterAsNum instanceof java.math.BigDecimal) {
				...
				try {
					...
					try {
						...
						throw SQLError.createSQLException("Can't set scale of '" + scale + "' for DECIMAL argument '"
								+ parameterAsNum + "'", SQLError.SQL_STATE_ILLEGAL_ARGUMENT, getExceptionInterceptor());
					}
				}
		...
		if (parameterObj == null) {
			...
			try {
				...
				case Types.LONGVARCHAR:
					if (parameterObj instanceof BigDecimal) {
						setString(parameterIndex, (StringUtils.fixDecimalExponent(StringUtils
				...
				case Types.BLOB:
					...
					if (parameterObj instanceof byte[]) {
						...
						setBytes(parameterIndex, StringUtils.getBytes(parameterObj.toString(), this.charConverter,
								this.charEncoding, this.connection.getServerCharacterEncoding(), this.connection
				...
				case Types.TIMESTAMP:
					...
					if (parameterObj instanceof String) {
						...
						java.text.DateFormat sdf = new java.text.SimpleDateFormat(getDateTimePattern(
								(String) parameterObj, false), Locale.US);
				...
				case Types.TIME:
					...
					if (parameterObj instanceof String) {
						java.text.DateFormat sdf = new java.text.SimpleDateFormat(getDateTimePattern(
								(String) parameterObj, true), Locale.US);
	...
	protected int setOneBatchedParameterSet(java.sql.PreparedStatement batchedStatement, int batchedParamIndex,
		...
		for (int j = 0; j < isNullBatch.length; j++) {
			if (isNullBatch[j]) {
				...
				if (isStreamBatch[j]) {
					batchedStatement.setBinaryStream(batchedParamIndex++, paramArg.parameterStreams[j],
							paramArg.streamLengths[j]);
				} else {
					((com.mysql.jdbc.PreparedStatement) batchedStatement).setBytesNoEscapeNoQuotes(batchedParamIndex++,
	...
	public void setString(int parameterIndex, String x) throws SQLException {
		...
		if (x == null) {
			...
			if (this.connection.isNoBackslashEscapesSet()) {
				...
				if (!needsHexEscape) {
					...
					if (!this.isLoadDataQuery) {
						parameterAsBytes = StringUtils.getBytes(quotedString.toString(), this.charConverter,
								this.charEncoding, this.connection.getServerCharacterEncoding(), this.connection
										.parserKnowsUnicode(), getExceptionInterceptor());
					} else {
					...
					if (!this.isLoadDataQuery) {
						parameterAsBytes = StringUtils.getBytes(x, this.charConverter, this.charEncoding,
								this.connection.getServerCharacterEncoding(), this.connection.parserKnowsUnicode(),
								getExceptionInterceptor());
					} else {
						// Send with platform character encoding
			...
			if (!this.isLoadDataQuery) {
				if (needsQuoted) {
					parameterAsBytes = StringUtils.getBytesWrapped(parameterAsString, '\'', '\'', this.charConverter,
							this.charEncoding, this.connection.getServerCharacterEncoding(), this.connection
									.parserKnowsUnicode(), getExceptionInterceptor());
				} else {
					parameterAsBytes = StringUtils.getBytes(parameterAsString, this.charConverter, this.charEncoding,
							this.connection.getServerCharacterEncoding(), this.connection.parserKnowsUnicode(),
							getExceptionInterceptor());
				}
			} else {
		...
		if (x == null) {
			...
			if (!this.useLegacyDatetimeCode) {
				...
				synchronized (sessionCalendar) {
					x = TimeUtil.changeTimezone(this.connection, sessionCalendar, targetCalendar, x, tz,
	 ...
	 *             if a database-access error occurs.
	 */
	private void setTimestampInternal(int parameterIndex, Timestamp x, Calendar targetCalendar, TimeZone tz,
		...
		if (x == null) {
			...
			if (!this.useLegacyDatetimeCode) {
				...
				Calendar sessionCalendar = this.connection.getUseJDBCCompliantTimezoneShift() ? this.connection
	...
	private synchronized void newSetDateInternal(int parameterIndex, Date x, Calendar targetCalendar)
			throws SQLException {
		if (this.ddf == null) {
	...
	private void doSSPSCompatibleTimezoneShift(int parameterIndex, Timestamp x, Calendar sessionCalendar)
			throws SQLException {
		Calendar sessionCalendar2 = (this.connection.getUseJDBCCompliantTimezoneShift()) ? this.connection
				.getUtcCalendar() : getCalendarInstanceForSessionOrNew();
	...
	private final void streamToBytes(Buffer packet, InputStream in, boolean escape, int streamLength, boolean useLength)
		...
		try {
			...
			if (this.connection.isNoBackslashEscapesSet()
					|| (this.connection.getUseUnicode() && connectionEncoding != null
							&& CharsetMapping.isMultibyteCharset(connectionEncoding) && !this.connection
							.parserKnowsUnicode())) {
				hexEscape = true;
			}
	...
	class EmulatedPreparedStatementBindings implements ParameterBindings {
		...
		public EmulatedPreparedStatementBindings() throws SQLException {
			...
			System.arraycopy(isNull, 0, this.parameterIsNull, 0, parameterCount);
			...
			for (int i = 0; i < parameterCount; i++) {
				...
				if (parameterTypes[i] == Types.BINARY || parameterTypes[i] == Types.BLOB) {
					charsetIndex = 63;
				} else {
					String mysqlEncodingName = CharsetMapping.getMysqlEncodingForJavaEncoding(connection.getEncoding(),
							connection);
					charsetIndex = CharsetMapping.getCharsetIndexForMysqlEncodingName(mysqlEncodingName);
				}

				Field parameterMetadata = new Field(null, "parameter_" + (i + 1), charsetIndex, parameterTypes[i],
						rowData[i].length);
			...
			this.bindingsAsRs = new ResultSetImpl(connection.getCatalog(), typeMetadata, new RowDataStatic(rows),
					connection, null);
	...
	protected static boolean canRewrite(String sql, boolean isOnDuplicateKeyUpdate, int locationOfOnDuplicateKeyUpdate,
		...
		if (isOnDuplicateKeyUpdate) {
			...
			if (updateClausePos != -1) {
				rewritableOdku = StringUtils.indexOfIgnoreCaseRespectMarker(updateClausePos, sql, "LAST_INSERT_ID",
						"\"'`", "\"'`", false) == -1;
			}
		}
