...
public class CharsetMapping {
	private static my.Debug DEBUG = new my.Debug(my.Debug.CharsetMapping);//我加上的
	...
	static {	
		...
		//左边是MySQL数据库自己的字符集名称，右边是最大字节个数
		//等同于"SHOW CHARACTER SET;"指令输出的结果，只不过这里多了最后一项"utf8mb4"
		...
		DEBUG.P("STATIC_CHARSET_TO_NUM_BYTES_MAP.size()="+STATIC_CHARSET_TO_NUM_BYTES_MAP.size());
		...
		DEBUG.P("JAVA_TO_MYSQL_CHARSET_MAP="+JAVA_TO_MYSQL_CHARSET_MAP);
		...
		DEBUG.P("MYSQL_TO_JAVA_CHARSET_MAP="+MYSQL_TO_JAVA_CHARSET_MAP);
		...
		//与JAVA_TO_MYSQL_CHARSET_MAP相同，只不过经过排序了，key也全是大写
		JAVA_UC_TO_MYSQL_CHARSET_MAP = Collections.unmodifiableMap(ucMap);

		DEBUG.P("JAVA_UC_TO_MYSQL_CHARSET_MAP="+JAVA_UC_TO_MYSQL_CHARSET_MAP);
		...
		DEBUG.P("MULTIBYTE_CHARSETS="+MULTIBYTE_CHARSETS);
		...
		DEBUG.P("MYSQL_ENCODING_NAME_TO_CHARSET_INDEX_MAP="+MYSQL_ENCODING_NAME_TO_CHARSET_INDEX_MAP);
		
		Map tempMap = new HashMap();
		
		//对应D:\Program Files\MySQL\MySQL Server 5.1\share目录中的子目录名
...
class VersionedStringProperty {
	...
	VersionedStringProperty(String property) {
		...
		//例如: UTF-8 =				*> 5.5.2 utf8mb4
		//在>=5.5.2以上版本优先使用utf8mb4
		...
		if (property.startsWith(">")) {
			...
			int charPos = 0; //查找Mysql实际字符集的第一个字符位置
