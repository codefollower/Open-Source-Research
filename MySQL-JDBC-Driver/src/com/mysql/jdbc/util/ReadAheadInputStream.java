...
public class ReadAheadInputStream extends InputStream {
	...
	public synchronized int read(byte b[], int off, int len) throws IOException {
		...
		//这里是一个技巧，"|"运算符是进行按位运算，注意不是布尔运算，
		//所以不像"||"运算符会发生短路计算，下面这一连串"|"运算都要计算完，
		//只要("|"运算符两边)任何一个是负值这一连串"|"运算的值就是负的，
		//这样就不用多个if条件来判断off<0或len<0等等多个分枝
	...
	public int read() throws IOException {
		...
		//InputStream.read()要求返回0-255范围内的的int 字节值
		//所以必须要“& 0xff”，这样就不会得到一个负值
