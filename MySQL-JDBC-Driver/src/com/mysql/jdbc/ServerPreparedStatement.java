...
public class ServerPreparedStatement extends PreparedStatement {
	private static my.Debug DEBUG=new my.Debug(my.Debug.ServerPreparedStatement);//我加上的
	...
	static class BatchedBindValues {
		//每一个BatchedBindValues实例就代表一次addBatch()，
		//batchedParameterValues中的每一个BindValue就代表一个setXXX进来的参数值，
		//数组的下标就是setXXX中的parameterIndex-1
		//比如ps.setInt(1, 100)
		//则batchedParameterValues[0] = BindValue(100);
	...
	public static class BindValue {
		...
		void reset() {
			//没有boundBeforeExecutionNum isNull bufferType bindLength
			this.isSet = false;
			this.value = null;
	...
	protected static ServerPreparedStatement getInstance(MySQLConnection conn, String sql, String catalog,
		...
		try {
			return (ServerPreparedStatement) JDBC_4_SPS_CTOR.newInstance(new Object[] { conn, sql, catalog,
					Constants.integerValueOf(resultSetType), Constants.integerValueOf(resultSetConcurrency) });
	 ...
	 *             If an error occurs
	 */
	protected ServerPreparedStatement(MySQLConnection conn, String sql, String catalog, int resultSetType,
			int resultSetConcurrency) throws SQLException {
		super(conn, catalog);

		try {//我加上的
		DEBUG.P(this,"ServerPreparedStatement(5)");
		DEBUG.P("sql="+sql);
		...
		DEBUG.P("this.hasOnDuplicateKeyUpdate="+this.hasOnDuplicateKeyUpdate );
		...
		DEBUG.P("this.serverNeedsResetBeforeEachExecution="+this.serverNeedsResetBeforeEachExecution);
		DEBUG.P("this.useAutoSlowLog="+this.useAutoSlowLog);
		DEBUG.P("this.useTrueBoolean="+this.useTrueBoolean);
		DEBUG.P("this.hasLimitClause="+this.hasLimitClause);
		DEBUG.P("this.originalSql="+this.originalSql);
		...
		try {
			...
			SQLException sqlEx = SQLError.createSQLException(ex.toString(), SQLError.SQL_STATE_GENERAL_ERROR,
		...
		DEBUG.P("this.parameterCount="+this.parameterCount);

		this.parameterTypes = new int[this.parameterCount];

		}finally{//我加上的
		DEBUG.P(0,this,"ServerPreparedStatement(5)");
		}
	}
	...
	public synchronized void addBatch() throws SQLException {
		...
		DEBUG.PA("parameterBindings",parameterBindings);

		this.batchedArgs.add(new BatchedBindValues(this.parameterBindings));

		}finally{//我加上的
		DEBUG.P(0,this,"addBatch()");
		}
	}
	...
	protected int[] executeBatchSerially(int batchTimeout) throws SQLException {
		try {//我加上的
		DEBUG.P(this,"executeBatchSerially(1)");
		DEBUG.P("batchTimeout="+batchTimeout);
								...
								try {
									...
									if (this.retrieveGeneratedKeys) {
										...
										try {
											...
											while (rs.next()) {
												this.batchedGeneratedKeys.add(new ByteArrayRow(new byte[][] { rs
		...
		DEBUG.P(0,this,"executeBatchSerially(1)");
		}
	}
	 ...
	 */
	//sendPacket、queryIsSelectOnly、isBatch没用到
	protected com.mysql.jdbc.ResultSetInternalMethods executeInternal(int maxRowsToRetrieve, Buffer sendPacket,
			boolean createStreamingResultSet, boolean queryIsSelectOnly, Field[] metadataFromCache, boolean isBatch)
			throws SQLException {
		try {//我加上的
		DEBUG.P(this,"executeInternal(...)");
		DEBUG.P("maxRowsToRetrieve="+maxRowsToRetrieve);
		DEBUG.P("sendPacket="+sendPacket);
		DEBUG.P("createStreamingResultSet="+createStreamingResultSet);
		DEBUG.P("queryIsSelectOnly="+queryIsSelectOnly);
		DEBUG.PA("metadataFromCache",metadataFromCache);
		DEBUG.P("isBatch="+isBatch);

		this.numberOfExecutions++; //executeInternal调用过多少次了

		DEBUG.P("numberOfExecutions="+numberOfExecutions);
		...
		try {
			...
			if (this.connection.getDumpQueriesOnException()) {
				...
				sqlEx = ConnectionImpl
						.appendMessageToException(sqlEx, messageBuf.toString(), getExceptionInterceptor());
			}
		...
		DEBUG.P(0,this,"executeInternal(...)");
		}
	}
	...
	protected BindValue getBinding(int parameterIndex, boolean forLongData) throws SQLException {
		...
		//在serverPrepare方法中会先在服务器端解析sql，看看有多少个?号
		//如果sql中没有?号，调用setXXX时会抛错，如下:
		//SQLState : S1009 
		...
		//parameterIndex是从1开始的，而数组下标从0开始
		parameterIndex--;

		if ((parameterIndex < 0) || (parameterIndex >= this.parameterBindings.length)) {
			
			//如:Parameter index out of bounds. 3 is not between valid values of 1 and 2
	...
	protected void rePrepare() throws SQLException {
		...
		try {
			...
			this.invalidationException = SQLError.createSQLException(ex.toString(), SQLError.SQL_STATE_GENERAL_ERROR,
					getExceptionInterceptor());
	...
	private com.mysql.jdbc.ResultSetInternalMethods serverExecute(int maxRowsToRetrieve,
		...
		synchronized (this.connection.getMutex()) {
			...
			if (mysql.shouldIntercept()) {
				ResultSetInternalMethods interceptedResults = mysql.invokeStatementInterceptorsPre(this.originalSql,
						this, true);
			...
			for (int i = 0; i < this.parameterCount; i++) {
				if (!this.parameterBindings[i].isSet) {
					throw SQLError.createSQLException(Messages.getString("ServerPreparedStatement.13") + (i + 1) //$NON-NLS-1$
							+ Messages.getString("ServerPreparedStatement.14"), SQLError.SQL_STATE_ILLEGAL_ARGUMENT,
							getExceptionInterceptor()); //$NON-NLS-1$
				}
			}
			...
			if (this.connection.versionMeetsMinimum(4, 1, 2)) {
				...
				if (this.resultFields != null && this.connection.isCursorFetchEnabled()
			...
			try {
				...
				if (logSlowQueries || gatherPerformanceMetrics) {
					...
					if (queryWasSlow) {
						...
						this.eventSink.consumeEvent(new ProfilerEvent(ProfilerEvent.TYPE_SLOW_QUERY,
								"", this.currentCatalog, this.connection.getId(), //$NON-NLS-1$
								getId(), 0, System.currentTimeMillis(), elapsedTime, mysql.getQueryTimingUnits(), null,
				...
				if (this.profileSQL) {
					...
					this.eventSink.consumeEvent(new ProfilerEvent(ProfilerEvent.TYPE_EXECUTE, "", this.currentCatalog, //$NON-NLS-1$
							this.connectionId, this.statementId, -1, System.currentTimeMillis(), (int) (mysql
									.getCurrentTimeNanosOrMillis() - begin), mysql.getQueryTimingUnits(), null,
							new Throwable(), truncateQueryToLog(asSql(true))));
				}

				com.mysql.jdbc.ResultSetInternalMethods rs = mysql.readAllResults(this, maxRowsToRetrieve,
						this.resultSetType, this.resultSetConcurrency, createStreamingResultSet, this.currentCatalog,
						resultPacket, true, this.fieldCount, metadataFromCache);

				if (mysql.shouldIntercept()) {
					ResultSetInternalMethods interceptedResults = mysql.invokeStatementInterceptorsPost(
							this.originalSql, this, rs, true, null);
				...
				if (this.profileSQL) {
					...
					this.eventSink.consumeEvent(new ProfilerEvent(
							...
							(fetchEndTime - queryEndTime), mysql.getQueryTimingUnits(), null, new Throwable(), null));
				}
	...
	//给数据库发COM_PREPARE命令及sql串，
	//数据库返回一个包(有语句id，字段个数(select语句才有)、参数个数)
	//然后再返回"参数个数+字段个数"这么多个包用来分别描述每个参数和字段的类型
	private void serverPrepare(String sql) throws SQLException {
		try {//我加上的
		DEBUG.P(this,"serverPrepare(1)");
		...
		synchronized (this.connection.getMutex()) {
			...
			try {
				...
				Buffer prepareResultPacket = mysql.sendCommand(MysqlDefs.COM_PREPARE, sql, null, false,
						characterEncoding, 0);

				DEBUG.P("prepareResultPacket="+prepareResultPacket);
				DEBUG.P("prepareResultPacket.getPosition()="+prepareResultPacket.getPosition());
				...
				DEBUG.P("serverStatementId="+serverStatementId);
				DEBUG.P("fieldCount="+fieldCount);
				DEBUG.P("parameterCount="+parameterCount);
				...
				if (this.profileSQL) {
					this.eventSink.consumeEvent(new ProfilerEvent(ProfilerEvent.TYPE_PREPARE, "", this.currentCatalog, //$NON-NLS-1$
							this.connectionId, this.statementId, -1, System.currentTimeMillis(), mysql
									.getCurrentTimeNanosOrMillis()
									- begin, mysql.getQueryTimingUnits(), null, new Throwable(),
							truncateQueryToLog(sql)));
				}

				if (this.parameterCount > 0) {
					if (this.connection.versionMeetsMinimum(4, 1, 2) && !mysql.isVersion(5, 0, 0)) {
						
						//并不是正常的字段名，这里只说明这个参数是什么样的mysql本地字段类型
						this.parameterFields = new Field[this.parameterCount];

						Buffer metaDataPacket = mysql.readPacket();

						DEBUG.P("metaDataPacket="+metaDataPacket);
						...
						DEBUG.PA("parameterFields",parameterFields);
					}
				}
				...
				if (this.fieldCount > 0) {
					...
					DEBUG.PA("resultFields",resultFields);
				}
			} catch (SQLException sqlEx) {
				if (this.connection.getDumpQueriesOnException()) {
		...
		DEBUG.P(0,this,"serverPrepare(1)");
		}
	}
	 ...
	 *             if a database access error occurs
	 */
	public void setTimeInternal(int parameterIndex, java.sql.Time x, Calendar targetCalendar, TimeZone tz,
		...
		if (x == null) {
			...
			if (!this.useLegacyDatetimeCode) {
				...
				synchronized (sessionCalendar) {
					binding.value = TimeUtil.changeTimezone(this.connection, sessionCalendar, targetCalendar, x, tz,
	...
	protected void setTimestampInternal(int parameterIndex, java.sql.Timestamp x, Calendar targetCalendar, TimeZone tz,
			boolean rollForward) throws SQLException {
		if (x == null) {
		...
		try {
			...
			case MysqlDefs.FIELD_TYPE_NEW_DECIMAL:
				if (value instanceof byte[]) {
					...
					packet.writeLenString((String) value, this.charEncoding, this.connection
							.getServerCharacterEncoding(), this.charConverter, this.connection.parserKnowsUnicode(),
							this.connection);
				} else {
	...
	private void storeDateTime412AndOlder(Buffer intoBuf, java.util.Date dt, int bufferType) throws SQLException {
		...
		if (!this.useLegacyDatetimeCode) {
			...
			sessionCalendar = (dt instanceof Timestamp && this.connection.getUseJDBCCompliantTimezoneShift()) ? this.connection
					.getUtcCalendar()
	...
	private void storeReader(MysqlIO mysql, int parameterIndex, Buffer packet, Reader inStream) throws SQLException {
		...
		try {
			...
			while ((numRead = inStream.read(buf)) != -1) {
				...
				byte[] valueAsBytes = StringUtils.getBytes(buf, null, clobEncoding, this.connection
						.getServerCharacterEncoding(), 0, numRead, this.connection.parserKnowsUnicode(),
						getExceptionInterceptor());
	...
	public synchronized boolean canRewriteAsMultiValueInsertAtSqlLevel() throws SQLException {
		if (!hasCheckedRewrite) {
			...
			this.canRewrite = canRewrite(this.originalSql, isOnDuplicateKeyUpdate(),
					getLocationOfOnDuplicateKeyUpdate(), 0);
			// We need to client-side parse this to get the VALUES clause, etc.
			this.parseInfo = new ParseInfo(this.originalSql, this.connection, this.connection.getMetaData(),
			...
			if (paramArg[j].isNull) {
				...
				if (paramArg[j].isLongData) {
					...
					if (value instanceof InputStream) {
						batchedStatement.setBinaryStream(batchedParamIndex++, (InputStream) value,
								(int) paramArg[j].bindLength);
					} else {
						batchedStatement.setCharacterStream(batchedParamIndex++, (Reader) value,
								(int) paramArg[j].bindLength);
					}
				} else {
					...
					case MysqlDefs.FIELD_TYPE_NEW_DECIMAL:
						...
						if (batchedStatement instanceof ServerPreparedStatement) {
							BindValue asBound = ((ServerPreparedStatement) batchedStatement).getBinding(
									batchedParamIndex, false);
	...
	protected PreparedStatement prepareBatchedInsertSQL(MySQLConnection localConn, int numBatches) throws SQLException {
		try {
			PreparedStatement pstmt = new ServerPreparedStatement(localConn, this.parseInfo.getSqlForBatch(numBatches),
					this.currentCatalog, this.resultSetConcurrency, this.resultSetType);
			pstmt.setRetrieveGeneratedKeys(this.retrieveGeneratedKeys);

			return pstmt;
		} catch (UnsupportedEncodingException e) {
			SQLException sqlEx = SQLError.createSQLException("Unable to prepare batch statement",
					SQLError.SQL_STATE_GENERAL_ERROR, getExceptionInterceptor());
			sqlEx.initCause(e);
