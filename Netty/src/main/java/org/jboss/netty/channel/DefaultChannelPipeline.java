...
public class DefaultChannelPipeline implements ChannelPipeline {
	private static my.Debug DEBUG=new my.Debug(my.Debug.DefaultChannelPipeline);//我加上的

    static final InternalLogger logger = InternalLoggerFactory.getInstance(DefaultChannelPipeline.class);
    static final ChannelSink discardingSink = new DiscardingChannelSink();

	//channel和sink在new出一个channel时会通过attach赋值，见AbstractChannel的构造函数
    ...
    public synchronized void addFirst(String name, ChannelHandler handler) {
		...
		DEBUG.P(0,this,"addFirst(2)");
		}
    }

	//依次调用addLast(h1)->addLast(h2)->addLast(h3)
	//得到h1-->h2-->h3
    public synchronized void addLast(String name, ChannelHandler handler) {
		try {//我加上的
		DEBUG.P(this,"addLast(2)");
		DEBUG.P("name="+name);
		DEBUG.P("name2ctx.isEmpty()="+name2ctx.isEmpty());
    ...
    public void sendUpstream(ChannelEvent e) {
		try {//我加上的
		DEBUG.P(this,"sendUpstream(1)");
		DEBUG.P("e="+e);
		DEBUG.P("this.head="+this.head);
		...
		DEBUG.P(0,this,"sendUpstream(1)");
		}
    }

    void sendUpstream(DefaultChannelHandlerContext ctx, ChannelEvent e) {
		try {//我加上的
		DEBUG.P(this,"sendUpstream(2)");

        try {
			//DEBUG.P("((ChannelUpstreamHandler) ctx.getHandler())="+((ChannelUpstreamHandler) ctx.getHandler()));

			DEBUG.P("next handler="+((ChannelUpstreamHandler) ctx.getHandler()).getClass().getName());
		...
		DEBUG.P(0,this,"sendUpstream(2)");
		}
    }

    public void sendDownstream(ChannelEvent e) {
		try {//我加上的
		DEBUG.P(this,"sendDownstream(1)");
		DEBUG.P("e="+e);
		DEBUG.P("this.tail="+this.tail);
		...
		DEBUG.P(0,this,"sendDownstream(1)");
		}
    }

    void sendDownstream(DefaultChannelHandlerContext ctx, ChannelEvent e) {
		try {//我加上的
		DEBUG.P(this,"sendDownstream(2)");
		DEBUG.P("((ChannelDownstreamHandler) ctx.getHandler())="+((ChannelDownstreamHandler) ctx.getHandler()));
		...
		DEBUG.P(0,this,"sendDownstream(2)");
		}
    }
    ...
    private final class DefaultChannelHandlerContext implements ChannelHandlerContext {
		...
		public String toString() {
			return "DefaultChannelHandlerContext[name="+name+", Upstream="+canHandleUpstream+", Downstream="+canHandleDownstream+", attachment="+attachment+", handler="+handler+"]";
		}
		...
		//从Handler1往Handler4是Upstream　　(从左到右)
		//从Handler4往Handler1是Downstream  (从右到左)
		//当调用pipeline.addLast时，第一次调用addLast的Handler排在最左，最后一次调用addLast的Handler排在最右
