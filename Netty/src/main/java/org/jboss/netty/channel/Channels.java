...
//以fire开头的方法都是发送upstream event
public class Channels {
	private static my.Debug DEBUG=new my.Debug(my.Debug.Channels);//我加上的
    ...
    public static void fireChannelOpen(Channel channel) {
		try {//我加上的
		DEBUG.P(Channels.class,"fireChannelOpen(1)");
		DEBUG.P("channel="+channel);
		DEBUG.P("channel.getClass()="+channel.getClass());
		DEBUG.P("channel.getParent()="+channel.getParent());

        // Notify the parent handler.
		//是指ServerSocket在accept之后，如得到一个OioAcceptedSocketChannel
        if (channel.getParent() != null) {
            fireChildChannelStateChanged(channel.getParent(), channel);
        }

		DEBUG.P("channel.getPipeline()="+channel.getPipeline());
        channel.getPipeline().sendUpstream(
                new UpstreamChannelStateEvent(
                        channel, ChannelState.OPEN, Boolean.TRUE));

		}finally{//我加上的
		DEBUG.P(0,Channels.class,"fireChannelOpen(1)");
		}
    }
    ...
    public static ChannelFuture bind(Channel channel, SocketAddress localAddress) {
		...
		DEBUG.P(0,Channels.class,"bind(2)");
		}
    }
    ...
    public static ChannelFuture connect(Channel channel, SocketAddress remoteAddress) {
		try {//我加上的
		DEBUG.P(Channels.class,"connect(2)");
		...
		DEBUG.P(0,Channels.class,"connect(2)");
		}
    }
    ...
    public static ChannelFuture write(Channel channel, Object message) {
		try {//我加上的
		DEBUG.P(Channels.class,"write(2)");
		DEBUG.P("channel="+channel);
		...
		DEBUG.P(0,Channels.class,"write(2)");
		}
    }
