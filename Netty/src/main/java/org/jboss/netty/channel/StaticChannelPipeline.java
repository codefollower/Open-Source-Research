...
public class StaticChannelPipeline implements ChannelPipeline {
	private static my.Debug DEBUG=new my.Debug(my.Debug.StaticChannelPipeline);//我加上的
    ...
    public StaticChannelPipeline(ChannelHandler... handlers) {
		try {//我加上的
		DEBUG.P(this,"StaticChannelPipeline(1)");
		DEBUG.PA("handlers",handlers);
		...
		DEBUG.P(0,this,"StaticChannelPipeline(1)");
		}
    }
	...
	//在调用SocketChannelFactory的子类的newChannel方法时，
	//把StaticChannelPipeline作为参数，在初始化一个Channel实例之后把
	//Channel实例及与这个Channel实例相关的ChannelSink一起挂到这个StaticChannelPipeline上
	//见AbstractChannel的构造函数
    public void attach(Channel channel, ChannelSink sink) {
		try {//我加上的
		DEBUG.P(this,"attach(2)");
		//不能打印NioServerSocketChannel类的(InetSocketAddress) socket.socket().getLocalSocketAddress();
		//socket未初始化
    ...
    void sendUpstream(StaticChannelHandlerContext ctx, ChannelEvent e) {
		...
		DEBUG.P("ctx.getHandler()="+ctx.getHandler());
	...
	//一个ChannelHandler单独对应一个StaticChannelHandlerContext
    private final class StaticChannelHandlerContext implements ChannelHandlerContext {
		...
		public String toString() {
			return "StaticChannelHandlerContext[index="+index+", name="+name+", Upstream="+canHandleUpstream+", Downstream="+canHandleDownstream+", attachment="+attachment+", handler="+handler+"]";
		}
