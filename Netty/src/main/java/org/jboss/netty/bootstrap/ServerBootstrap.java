...
public class ServerBootstrap extends Bootstrap {
	private static my.Debug DEBUG=new my.Debug(my.Debug.ServerBootstrap);//我加上的
    ...
    public Channel bind(final SocketAddress localAddress) {
        ...
        do {
            try {
				DEBUG.P("futureQueue.size()="+futureQueue.size());
				//阻塞等待Integer.MAX_VALUE秒，在Binder中如果收到channel Open事件
				//会把一个future(由bind或由exception产生)加到futureQueue中
		...
		DEBUG.P("(!future.isSuccess())="+(!future.isSuccess()));
        if (!future.isSuccess()) {
    ...
    private final class Binder extends SimpleChannelUpstreamHandler {
        ...
        Binder(SocketAddress localAddress, BlockingQueue<ChannelFuture> futureQueue) {
            this.localAddress = localAddress; //ServerSocket的绑定地址，比如:0.0.0.0/0.0.0.0:8080
        ...
        public void channelOpen(
            ...
            try {
				...
				DEBUG.P("allOptions="+allOptions);
				DEBUG.P("parentOptions="+parentOptions);
				DEBUG.P("childOptions="+childOptions);
			...
			DEBUG.P("evt.getChannel().getClass()="+evt.getChannel().getClass());

			ChannelFuture bindFuture;

            boolean finished = futureQueue.offer(bindFuture = evt.getChannel().bind(localAddress));

			DEBUG.P("finished="+finished);
			DEBUG.P("bindFuture="+bindFuture);
            assert finished;

			}finally{//我加上的
			DEBUG.P(0,this,"channelOpen(2)");
			}
        }
